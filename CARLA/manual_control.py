        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if isinstance(self._control, carla.VehicleControl):
                    # Cuando ponga reversa cambie la camara
                    if event.key == K_q:
...
#        print(fonts)
        ...
        self.help = HelpText(pygame.font.Font(mono, 15), width, height)
        self.server_fps = 0
        self.frame_number = 0
...
#            'Map:     % 20s' % world.map.name,
            'Tiempo de restante: % 12s' % datetime.timedelta(seconds=int(5*60-(time.time()-self.timer) + 2)),#self.simulation_time
#            '',
...
#            'Height:  % 18.0f m' % t.location.z,
            '']
#        self._infoTime = ['Tiempo de simulación: % 12s' % datetime.timedelta(seconds=int(self.simulation_time))]
...
#            elif (velocidad > 30):
#                s2.play(-1)
                #'Traffic Lights:   %s' % len(traffic_lights)]   
...
#                pygame.FULLSCREEN)
        ...
        s3.set_volume(0.05)
        s4 = pygame.mixer.Sound("5_constant.wav")
        s4.set_volume(0.05)
...
#                print('spawned %r at %s' % (vehicle.type_id, transform.location))
        ...
        while True:
            ...
            # Pasado un segundo, calcule la diferencia de velocidad
            if  int(round(time.time()-ini,1)*10) == 10:#(int(time.time()-ini))
                flag = 0
...
##                print('no 1')
            ...
            if round(time.time()-start)-1 >= 5*60:
                # MENSAJE DE FIN
                default_font = 'monotypecorsiva'
                ...
                display.blit(surface, (round(args.height/2),round(args.width/2)))# 8
